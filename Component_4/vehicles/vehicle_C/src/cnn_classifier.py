# vehicles/vehicle_C/src/cnn_classifier.py
# Step 2: CNN Classification + Knowledge Labeling - Vehicle C
...
CNN_MODEL_PATH = "vehicles/vehicle_C/models/vehicle_C_cnn_danger.keras"
CLASS_MAPPING_PATH = "vehicles/vehicle_C/models/vehicle_C_class_mapping.json"
...
class VehicleCCNN:
    ...
    def __init__(self):
        """
        Load Vehicle C CNN model and class mapping.
        """
        ...
        try:
            ...
            print("Vehicle C CNN classifier loaded.")

        except Exception as e:
            print("Failed to load Vehicle C CNN classifier.")
            print(e)
    ...
    def classify(self, cropped_sign):
        ...
        # Get CNN output index and confidence
        predicted_index = int(np.argmax(prediction))
        confidence = float(np.max(prediction))

        # Get original GTSRB class information
        mapping = self.class_mapping[str(predicted_index)]

        class_id = mapping["original_class_id"]
        class_name = mapping["class_name"]

        # Assign knowledge status
        status = self.get_status(confidence)

        # Decide whether sign continues in pipeline
        if status == "KNOWN":
            action = "IGNORE"
        else:
