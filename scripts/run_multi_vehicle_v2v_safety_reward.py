#!/usr/bin/env python3
"""
SUMO TraCI Control Script: Multi-Vehicle Dynamic Priority Intent with V2V Communication, Safety Monitor, and Reward Calculation
This script implements a multi-vehicle dynamic priority intent algorithm with simulated V2V communication,
safety override layer, and reward calculation for autonomous vehicles at unsignalized intersections.
...
The reward calculation layer provides a quantitative evaluation of vehicle behavior for
future reinforcement learning training. It calculates rewards based on safety, efficiency,
and cooperation metrics that are essential for multi-agent autonomous vehicle systems.

Author: Research Project on Autonomous Vehicle Right-of-Way Negotiation with V2V Communication, Safety, and Rewards
"""
...
# Reward Function Parameters - Essential for reinforcement learning
# These parameters define the reward structure for autonomous vehicle behavior
COMPLETION_REWARD = 100  # Reward for successful route completion per vehicle
COLLISION_PENALTY = -1000  # Penalty for collision events
SAFETY_OVERRIDE_PENALTY = -50  # Penalty for safety override events
SAFETY_WARNING_PENALTY = -10  # Penalty for safety warning events
WAITING_PENALTY_PER_SECOND = -0.1  # Penalty per second of waiting time
SAFE_YIELDING_REWARD = 5  # Reward for safe yielding behavior
PRIORITY_CLEAR_REWARD = 10  # Reward for priority vehicle clearing intersection safely
DEADLOCK_PENALTY = -20  # Penalty for deadlock/stuck situations
DEADLOCK_SPEED_THRESHOLD = 0.5  # Speed threshold for deadlock detection (m/s)
DEADLOCK_TIME_THRESHOLD = 5.0  # Time threshold for deadlock detection (seconds)
...
# These variables track reward-related events and statistics for reinforcement learning
vehicle_rewards = {
    "vehicle_A": 0,
    "vehicle_B": 0,
    "vehicle_C": 0,
...
successful_yielding_events = 0
priority_clear_events = 0
completion_events = 0
deadlock_events = 0
collision_events = 0
reward_print_interval = 10  # Print reward info every 10 steps (1 second)
rewarded_priority_clear_vehicles = set()  # Track vehicles that have received priority clear reward to avoid duplicates
...
def detect_deadlocks():
    """
    Detect deadlock situations where vehicles are stuck for extended periods.
    This is important for reward calculation and system robustness.
    Returns list of vehicles in deadlock.
    """
    deadlock_vehicles = []
    ...
    try:
        ...
        for vehicle_id in all_vehicles:
            # Skip cleared vehicles
            if vehicle_id in vehicles_cleared_intersection:
                continue
            
            # Check if vehicle has been stuck
            if vehicle_id in vehicle_states:
                state = vehicle_states[vehicle_id]
                speed = get_vehicle_speed(vehicle_id)
                
                # Check for deadlock conditions
                if speed < DEADLOCK_SPEED_THRESHOLD:
                    stuck_time = current_time - state.get('last_moving_time', current_time)
                    if stuck_time > DEADLOCK_TIME_THRESHOLD:
                        deadlock_vehicles.append(vehicle_id)
                else:
                    # Update last moving time
                    state['last_moving_time'] = current_time
        
        return deadlock_vehicles
        
    except traci.TraCIException:
        return []

def apply_waiting_penalties():
    """
    Apply waiting penalties to vehicles that are yielding or stopped.
    This encourages efficient movement and discourages excessive waiting.
    """
    global total_system_reward
    ...
    try:
        ...
        for vehicle_id in all_vehicles:
            ...
            # Check if vehicle is yielding or stopped
            speed = get_vehicle_speed(vehicle_id)
            if speed < yielding_speed:  # Vehicle is effectively stopped or yielding
                penalty = WAITING_PENALTY_PER_SECOND * STEP_LENGTH
                vehicle_rewards[vehicle_id] += penalty
                total_system_reward += penalty
        
    except traci.TraCIException:
        pass

def apply_safety_penalties_and_rewards():
    """
    Apply safety-related penalties and rewards based on recent safety events.
    This integrates safety monitoring with the reward system.
    """
    global total_system_reward, successful_yielding_events, priority_clear_events
    
    try:
        all_vehicles = traci.vehicle.getIDList()
        
        # Apply safety override penalties to involved vehicles
        if safety_override_count > 0:
            for vehicle_id in all_vehicles:
                if vehicle_id not in vehicles_cleared_intersection:
                    penalty = SAFETY_OVERRIDE_PENALTY * STEP_LENGTH
                    vehicle_rewards[vehicle_id] += penalty
                    total_system_reward += penalty
        
        # Apply safety warning penalties to involved vehicles
        if safety_warning_count > 0:
            for vehicle_id in all_vehicles:
                if vehicle_id not in vehicles_cleared_intersection:
                    penalty = SAFETY_WARNING_PENALTY * STEP_LENGTH
                    vehicle_rewards[vehicle_id] += penalty
                    total_system_reward += penalty
        
        # Apply safe yielding rewards
        if current_priority_vehicle and current_yielding_vehicles:
            # Check if priority vehicle is moving forward safely
            priority_speed = get_vehicle_speed(current_priority_vehicle)
            if priority_speed > yielding_speed:
                # Reward yielding vehicles for safe cooperation
                for yielding_vehicle in current_yielding_vehicles:
                    if yielding_vehicle in vehicle_rewards:
                        reward = SAFE_YIELDING_REWARD * STEP_LENGTH
                        vehicle_rewards[yielding_vehicle] += reward
                        total_system_reward += reward
                        successful_yielding_events += 1
        
        # Apply priority clear rewards
        # Check for vehicles that have PRIORITY status and have cleared the intersection
        for vehicle_id in vehicles_cleared_intersection:
            # Check if this vehicle had priority status and hasn't been rewarded yet
            if vehicle_id in vehicle_states and vehicle_id not in rewarded_priority_clear_vehicles:
                # Check if this vehicle was the priority vehicle when it cleared
                # We can determine this by checking if it's in the priority order
                if vehicle_id in priority_order:
                    # Reward priority vehicle for clearing intersection safely
                    if vehicle_id in vehicle_rewards:
                        reward = PRIORITY_CLEAR_REWARD
                        vehicle_rewards[vehicle_id] += reward
                        total_system_reward += reward
                        priority_clear_events += 1
                        rewarded_priority_clear_vehicles.add(vehicle_id)
                        print(f"  [REWARD] {vehicle_id} received priority clear reward (+{reward})")
        
    except traci.TraCIException:
        pass

def apply_deadlock_penalties():
    """
    Apply deadlock penalties to vehicles that are stuck for extended periods.
    This discourages inefficient negotiation and encourages system robustness.
    """
    global total_system_reward, deadlock_events
    
    deadlock_vehicles = detect_deadlocks()
    
    for vehicle_id in deadlock_vehicles:
        if vehicle_id in vehicle_rewards:
            penalty = DEADLOCK_PENALTY * STEP_LENGTH
            vehicle_rewards[vehicle_id] += penalty
            total_system_reward += penalty
            deadlock_events += 1
...
def apply_completion_rewards():
    """
    Apply completion rewards when vehicles successfully complete their routes.
    This provides positive reinforcement for successful navigation.
    """
    global total_system_reward, completion_events
    
    try:
        all_vehicles = traci.vehicle.getIDList()
        
        # Check for newly completed vehicles
        for vehicle_id in all_vehicles:
            if vehicle_id in vehicles_cleared_intersection:
                # Check if this is a new completion
                if vehicle_id in vehicle_states and not vehicle_states[vehicle_id].get('completion_rewarded', False):
                    if vehicle_id in vehicle_rewards:
                        reward = COMPLETION_REWARD
                        vehicle_rewards[vehicle_id] += reward
                        total_system_reward += reward
                        completion_events += 1
                        vehicle_states[vehicle_id]['completion_rewarded'] = True
...
def update_rewards():
    """
    Update all reward calculations for the current simulation step.
    This is the main reward calculation function called every simulation step.
    """
    # Apply waiting penalties
    apply_waiting_penalties()
    
    # Apply safety penalties and rewards
    apply_safety_penalties_and_rewards()
    
    # Apply deadlock penalties
    apply_deadlock_penalties()
...
def broadcast_v2v_message(vehicle_id):
    ...
    # Get current reward for this vehicle
    reward_so_far = vehicle_rewards.get(vehicle_id, 0)
    
    # Create V2V message structure with safety status and reward
    v2v_messages[vehicle_id] = {
        ...
        "reward_so_far": reward_so_far  # Added reward tracking
    }
...
def print_v2v_messages():
    ...
    for vehicle_id in sorted(v2v_messages.keys()):
        ...
        print(f"    reward_so_far: {message['reward_so_far']:.1f}")
...
def print_reward_information():
    """
    Print current reward information for all vehicles.
    This provides detailed reward tracking information for reinforcement learning.
    """
    current_time = traci.simulation.getTime()
    
    print(f"\n=== Reward Calculation Information at {current_time:.1f}s ===")
    print(f"  Total system reward: {total_system_reward:.1f}")
    print(f"  Successful yielding events: {successful_yielding_events}")
    print(f"  Priority clear events: {priority_clear_events}")
    print(f"  Completion events: {completion_events}")
    print(f"  Deadlock events: {deadlock_events}")
    print(f"  Collision events: {collision_events}")
    
    print(f"  Vehicle rewards:")
    for vehicle_id in EXPECTED_VEHICLES:
        reward = vehicle_rewards.get(vehicle_id, 0)
        print(f"    {vehicle_id}: {reward:.1f}")
    
    # Print recent reward events
    print(f"  Reward function parameters:")
    print(f"    Completion reward: {COMPLETION_REWARD}")
    print(f"    Collision penalty: {COLLISION_PENALTY}")
    print(f"    Safety override penalty: {SAFETY_OVERRIDE_PENALTY}")
    print(f"    Safety warning penalty: {SAFETY_WARNING_PENALTY}")
    print(f"    Waiting penalty per second: {WAITING_PENALTY_PER_SECOND}")
    print(f"    Safe yielding reward: {SAFE_YIELDING_REWARD}")
    print(f"    Priority clear reward: {PRIORITY_CLEAR_REWARD}")
    print(f"    Deadlock penalty: {DEADLOCK_PENALTY}")
...
def initialize_vehicle_state(vehicle_id):
    ...
    if vehicle_id not in vehicle_states:
        vehicle_states[vehicle_id] = {
            ...
            'priority_intent': 0.0,
            'is_normal': True,
            'last_moving_time': traci.simulation.getTime(),  # For deadlock detection
            'completion_rewarded': False  # For reward tracking
        }
...
def print_simulation_status():
    """Print comprehensive current simulation status with V2V communication, safety, and reward information."""
    current_time = traci.simulation.getTime()
    ...
    try:
        ...
        for vehicle_id in sorted(all_vehicles):
            ...
            if vehicle_id in vehicle_states:
                ...
                print(f"    Reward so far: {reward:.1f}")
...
def print_final_summary():
    """Print a comprehensive final summary with V2V communication, safety, and reward statistics."""
    current_time = traci.simulation.getTime()
    
    print("\n" + "="*80)
    print("MULTI-VEHICLE DYNAMIC PRIORITY INTENT WITH V2V COMMUNICATION, SAFETY MONITOR, AND REWARD CALCULATION SUMMARY")
    print("="*80)
...
def run_simulation():
    """Main simulation loop with V2V communication, safety monitor, reward calculation, and multi-vehicle dynamic priority intent-based control logic."""
    global simulation_start_time
    
    print("=== Starting Multi-Vehicle Dynamic Priority Intent Simulation with V2V Communication, Safety Monitor, and Reward Calculation ===")
    ...
    print("Reward Layer: Quantitative evaluation for future reinforcement learning training")
...
def main():
    """Main function to run multi-vehicle dynamic priority intent simulation with V2V communication, safety monitor, and reward calculation."""
    try:
        import traci
    except ImportError:
